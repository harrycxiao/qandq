#CLI LLM agent to acces the API endpoints for Postgres for JSON data
# Make sure to set:
...
agent = Agent(
    ...
    system_prompt="""
You are an assistant that interacts with
a workbook backend.
...
@agent.tool_plain
def get_record_by_name_tool(
    name:str
):

    response=requests.get(
        f"{BASE_URL}/records/{name}"
    )
